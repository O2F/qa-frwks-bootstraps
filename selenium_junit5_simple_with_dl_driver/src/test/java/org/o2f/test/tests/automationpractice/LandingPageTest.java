...
public class LandingPageTest extends TestBaseLifeCycle {
    ...
    void shouldBePossibleToSearch(TestInfo testInfo) {
        try {
            ...
            reporter(testInfo, Status.FAIL, e.getMessage());
            throw e;
        }
    }
    ...
    void anotherTest(TestInfo testInfo){
        try {
            log.info("Starting Search 2 Test");
            // This is just for the sake of the extended reports usage testing
            Assertions.fail();
            reporter(testInfo);
        }catch (Exception | Error e){
            reporter(testInfo, Status.FAIL, e.toString());
            throw e;
        }
    }
